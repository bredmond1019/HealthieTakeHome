...
# Association Table for Client|Provider|Plan
...
class JournalEntry(db.Model):
    ...
    client_id = db.Column(db.Integer, db.ForeignKey("client.id"))
    date_entered = db.Column(db.DateTime(), default=datetime.today().replace(microsecond=0))
    ...
    def to_dict(self):
        data = {
            ...
            "date" : self.date_entered.isoformat()
        }
        return data


    def __repr__(self):
        return f"<Journal Entry -- client: {self.client_id}, date: {self.date_entered}, text: {self.entry[:10]}>"
